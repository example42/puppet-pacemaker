#
# = Define: pacemaker::conf
#
# With this define you can manage any pacemaker configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/pacemaker/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/pacemaker/my.conf',
#
# [*ensure*]
...
define pacemaker::conf (
  ...
  $manage_path    = pickx($path, "${pacemaker::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $pacemaker::config_file_mode)
  $manage_owner   = pickx($owner, $pacemaker::config_file_owner)
  $manage_group   = pickx($group, $pacemaker::config_file_group)
  $manage_require = pickx($config_file_require, $pacemaker::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $pacemaker::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "pacemaker_conf_${name}":
    ensure  => $ensure,
    source  => $source,
